...
import numpy as np
import yaml
from .modules.diffusionmodules.discretizer import Img2ImgDiscretizationWrapper, Txt2NoisyDiscretizationWrapper
"""
...
"""
from .modules.diffusionmodules.sampler import PanGuEulerEDMSampler
from .util import get_obj_from_str, instantiate_from_config, seed_everything
...
def load_model_from_config(model_config, ckpt=None, verbose=True):
    model = instantiate_from_config(model_config)
    sd = utils.load_torch_file(ckpt)
...
def pangu_get_batch(keys, value_dict, N: Union[List, ListConfig], dtype=torch.float32,device="cuda"):
    # Hardcoded demo setups; might undergo some changes in the future
    ...
    for key in keys:
        if key == "txt":
            ...
            batch["original_size_as_tuple"] = torch.from_numpy(
                np.tile(
                    np.array([value_dict["orig_height"], value_dict["orig_width"]]),
                    N_reshape
                    + [
            ...
            batch["crop_coords_top_left"] = torch.from_numpy(
                np.tile(
                    np.array([value_dict["crop_coords_top"], value_dict["crop_coords_left"]]),
                    N_reshape
                    + [
            ...
            batch["aesthetic_score"] = torch.from_numpy(
                np.tile(
                    np.array([value_dict["aesthetic_score"]]),
                    N_reshape
                    + [
            ...
            batch_uc["aesthetic_score"] = torch.from_numpy(
                np.tile(
                    np.array([value_dict["negative_aesthetic_score"]]),
                    N_reshape
                    + [
            ...
            batch["target_size_as_tuple"] = torch.from_numpy(
                np.tile(
                    np.array([value_dict["target_height"], value_dict["target_width"]]),
                    N_reshape
                    + [
            ...
            batch["target_size_as_ind"] = torch.from_numpy(
                np.tile(
    ...
    if "anime_scale" in value_dict and value_dict["anime_scale"] > 0:
        ...
        for key in batch.keys():
            if isinstance(batch[key], Tensor):
                batch_anime[key] = batch[key].clone()
        batch_anime["txt"] = (
            np.repeat(
    ...
    if "photography_scale" in value_dict and value_dict["photography_scale"] > 0:
        ...
        for key in batch.keys():
            if isinstance(batch[key], Tensor):
                batch_photography[key] = batch[key].clone()
...
def pangu_get_guider(guider="PanGuVanillaCFG", cfg_scale=7.5, other_scale=None):
    if guider == "IdentityGuider":
        ...
        dyn_thresh_config = {"target": "sgm.modules.diffusionmodules.sampling_utils.PanGuNoDynamicThresholding"}
        guider_config = {
            "target": "sgm.modules.diffusionmodules.guiders.PanGuVanillaCFG",
