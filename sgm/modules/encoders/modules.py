...
from transformers import (ByT5Tokenizer, CLIPTextModel, CLIPTokenizer,
                          T5EncoderModel, T5Tokenizer,CLIPTextConfig)
...
class GeneralConditioner(nn.Module):
        ...
        for i in range(len(self.embedders)):
            ...
            for emb in emb_out:
                if embedder.ucg_rate > 0.0 and embedder.legacy_ucg_val is None:
                    emb = (
                        expand_dims_like(
                            torch.bernoulli((1.0 - embedder.ucg_rate) * torch.ones(emb.size[0], dtype=emb.dtype)),
                            emb,
                        )
                        * emb
                    )
    ...
    def tokenize_embedding(self, batch: Dict, force_zero_embeddings: Optional[List] = None) -> Dict:
        ...
        #is_tensor_list = all(torch.is_tensor(t) for t in tokens)
        #if not is_tensor_list:
        #    tokens = [torch.from_numpy(t) for t in tokens]
...
class FrozenCnCLIPEmbedder(AbstractEmbModel):
    ...
    def __init__(
        ...
        self.transformer = CLIPTextModel(config=config)
        """
    ...
    def tokenize(self, texts):
        ...
        result_tensor = torch.from_numpy(result).to(self.device)
...
class FrozenOpenCLIPEmbedder2(AbstractEmbModel):
    ...
    def forward(self, text):
        ...
        z = self.encode_with_transformer(text.to(self.device))
