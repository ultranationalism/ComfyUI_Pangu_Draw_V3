...
class BaseDiffusionSampler:
    ...
    def prepare_sampling_loop(self, x, cond, uc=None, num_steps=None):
        ...
        x *= tensor(np.sqrt(1.0 + sigmas[0] ** 2.0), dtype=x.dtype,device="cuda")
        num_sigmas = len(sigmas)

        s_in = torch.ones((x.shape[0],), dtype=x.dtype,device="cuda")
    ...
    def denoise(self, x, model, sigma, cond, uc, **kwargs):
        ...
        model_output = model.model(
            (noised_input * c_in).float(), c_noise.int32(), **cond, **kwargs
        )
...
class SingleStepDiffusionSampler(BaseDiffusionSampler):
    def sampler_step(self, sigma, next_sigma, model, x, cond, uc=None, gamma=0.0, **kwargs):
        ...
        if gamma > 0:
            eps = tensor(np.random.randn(*x.shape), dtype=x.dtype,device="cuda") * self.s_noise
...
class AncestralSampler(SingleStepDiffusionSampler):
    def __init__(self, eta=1.0, s_noise=1.0, *args, **kwargs):
        ...
        self.noise_sampler = lambda x: tensor(np.random.randn(*x.shape), dtype=x.dtype)
...
class EDMSampler(SingleStepDiffusionSampler):
    ...
    def sampler_step(self, sigma, next_sigma, model, x, cond, uc=None, gamma=0.0, **kwargs):
        sigma_hat = sigma * (gamma + 1.0)
        if gamma > 0:
            eps = tensor(np.random.randn(*x.shape), dtype=x.dtype) * self.s_noise
            x = x + eps * append_dims(sigma_hat**2 - sigma**2, x.ndim) ** 0.5
...
class PanGuEDMSampler(SingleStepDiffusionSampler):
    ...
    def denoise(self, x, model, sigma, cond, uc, other_c, **kwargs):
        ...
        #cond = model.openai_input_warpper(cond)
        #print(type(cond.get("concat", torch.Tensor([]).type_as(x))))
        denoised = model.denoiser(model.model,noised_input,sigmas,cond,**kwargs)
        """
        model_output = model.model(
            (noised_input * c_in).float(), c_noise.int32(), **cond, **kwargs
        )
    ...
    def sampler_step(self, sigma, next_sigma, model, x, cond, uc=None, other_c=None, gamma=0.0, **kwargs):
        ...
        step_idx = model.denoiser.sigma_to_idx(sigma_hat).detach().cpu().numpy().item()
        d = to_d(x, sigma_hat, denoised)
        dt = append_dims(next_sigma - sigma_hat, x.ndim)
